...
def process_data(images, boxes=None):
    ...
    orig_size = [np.expand_dims(s, axis=0) for s in orig_size]
    
    # Image preprocessing.
    processed_images = [cv2.resize(i, (416, 416), cv2.INTER_CUBIC).astype('float32')/255. for i in images]
    ...
    if boxes is not None:
        ...
        boxes_xy = [boxxy / o for boxxy, o in zip(boxes_xy, orig_size)]
        boxes_wh = [boxwh / o for boxwh, o in zip(boxes_wh, orig_size)]
...
def train(model, class_names, anchors, image_data, boxes, detectors_mask, matching_true_boxes, stage_1_epoch=5,
    ...
    model_save.save('trained_stage_1.h5')
    
    model_body, model = create_model(anchors, class_names, load_pretrained=False, freeze_body=False)

    model.load_weights('trained_stage_1_w.h5')
